...
public class UnifiedLogQueryServiceImpl implements UnifiedLogQueryService {
	...
	private boolean matchesFilter(UnifiedLog log, UnifiedLogFilterRequest filter) {
		if (filter.traceId() != null && !filter.traceId().equals(log.getTraceId())) return false;
		if (filter.userId() != null && !filter.userId().equals(log.getUserId())) return false;
		if (filter.merchantId() != null && !filter.merchantId().equals(log.getMerchantId())) return false;

		if (filter.from() != null && log.getTimestamp().isBefore(filter.from().atStartOfDay())) return false;
		if (filter.to() != null && log.getTimestamp().isAfter(filter.to().plusDays(1).atStartOfDay().minusNanos(1))) return false;

		if (filter.logTypes() != null && !filter.logTypes().contains(log.getLogType())) return false;
